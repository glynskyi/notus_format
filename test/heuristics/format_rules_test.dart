...
void main() {
  group('$ResolveLineFormatRule', () {
    ...
    test('apply with zero length in the middle of a line', () {
      final ul = NotusAttribute.ul.toJson();
      final doc = Delta()..insert('Title\nOne')..insert('\n', ul)..insert('Two')..insert('\n', ul)..insert('Three!\n');
  ...
  group('$ResolveInlineFormatRule', () {
    ...
    test('apply', () {
      ...
      final expected = Delta()..retain(7, b)..retain(1)..retain(4, b)..retain(1)..retain(5, b)..retain(1)..retain(1, b);
      expect(actual, expected);
    });
  });
  ...
  group('$FormatLinkAtCaretPositionRule', () {
    ...
    test('apply', () {
      final link = NotusAttribute.link.fromString('https://github.com/memspace/bold');
      final newLink = NotusAttribute.link.fromString('https://github.com/memspace/zefyr');
      final doc = Delta()..insert('Visit our ')..insert('website', link.toJson())..insert(' for more details.\n');
